...
export async function importRequest(request: Request): Promise<ImportRequest | null> {
  ...
  if (typeof body?.source !== "string" || !body.source.trim() || body.source.length > 20_000)
    return null;
...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    if (request.headers.get("Authorization") !== `Bearer ${env.IMPORT_AI_SHARED_SECRET}`)
        ...
        messages: [
          {
            role: "system",
            content:
              'Extrahiere ausschließlich belegbare Angaben aus Stellenanzeigen. Erfinde nichts. Antworte ausschließlich mit einem JSON-Objekt ohne Markdown: {"company":string,"role":string,"salary":string|null,"deadlineAt":"YYYY-MM-DD"|null,"notes":string}.',
          },
          {
            role: "user",
            content: `Extrahiere company und role. Nutze deadlineAt nur bei eindeutigem Datum im Format YYYY-MM-DD, sonst null. notes fasst Aufgaben, Anforderungen und Arbeitsort knapp zusammen.\n\n${input.source}`,
          },
        ],
      });
