...
export async function POST(request: Request) {
  ...
  const body = await json(request),
    company = text(body?.company, 160, true),
    role = text(body?.role, 160, true),
    notes = text(body?.notes, 8000) ?? "",
    jobUrl = text(body?.jobUrl, 2048),
    salary = text(body?.salary, 120),
  ...
  if (
    ...
    (status !== undefined && !applicationStatusValues.includes(status as never))
  )
    return denied(400, "Invalid application data");
  return Response.json(
    {
      application: await createApplication(user.id, {
        company,
        role,
        notes,
        jobUrl: jobUrl || null,
