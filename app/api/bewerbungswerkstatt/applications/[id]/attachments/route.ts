...
export async function POST(request: Request, { params }: Context) {
  ...
  if (
    ...
    return denied(400, "Bitte lade eine PDF-Datei bis 10 MB hoch.");
  const bytes = new Uint8Array(await file.slice(0, 5).arrayBuffer());
  if (new TextDecoder().decode(bytes) !== "%PDF-") return denied(400, "Ungültige PDF-Datei.");
  const attachment = await addAttachment(user.id, (await params).id, kind as never, file);
  return attachment
