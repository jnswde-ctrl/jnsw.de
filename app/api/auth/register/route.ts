...
export async function POST(request: Request) {
  ...
  if (
    ...
    return Response.json(
      {
        error: "Use a valid email, a display name, and a password with at least 12 characters",
      },
      { status: 400 },
    );
  ...
  try {
    ...
    return Response.json(
      {
        error:
          "Dein Konto wurde angelegt, aber die Bestätigungs-E-Mail konnte nicht gesendet werden. Bitte versuche die Anmeldung später erneut.",
      },
      { status: 503 },
    );
  }
  return Response.json(
    {
      message:
        "Dein Konto ist angelegt. Bitte bestätige jetzt die E-Mail-Adresse über den Link in deinem Postfach.",
    },
    { status: 201 },
  );
}
