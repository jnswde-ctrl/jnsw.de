...
export function PdfWorkbench() {
  ...
  const addFiles = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setItems((current) => addUniqueFiles(current, selected.filter(isPdfFile)).items);
    event.target.value = "";
  };

  return (
    <>
      <div className="workspace-modes" role="tablist" aria-label="PDF-Funktionen">
        <button
          type="button"
          role="tab"
      ...
      {mode === "select" && (
        <section className="workspace-file-selection" aria-labelledby="workspace-files-title">
          <p className="eyebrow">Schritt 01 / Dateien</p>
          <h3 id="workspace-files-title">Welche PDFs möchtest du bearbeiten?</h3>
          <p>
            Wähle die Dateien aus. Danach entscheidest du, ob du sie verbinden oder umbenennen
            möchtest.
          </p>
          <input
            ref={inputRef}
          ...
          {items.length > 0 && (
            <div className="workspace-selected-summary">
              <span>
                {items.length} PDF-Datei{items.length === 1 ? "" : "en"} ausgewählt
              </span>
      ...
      {mode === "story" && (
        <section className="workspace-story" aria-labelledby="workspace-story-title">
          ...
          <p className="workspace-story-lead">
            Die PDF-Werkbank löst zwei wiederkehrende Aufgaben: Dokumente lokal verbinden und
            Dateinamen nachvollziehbar vorbereiten.
          </p>
          <dl>
            <div>
              <dt>Problem</dt>
              <dd>
                Viele PDF-Dienste verlangen für einfache Arbeitsabläufe einen Upload oder ein Konto.
              </dd>
            </div>
            <div>
              <dt>Entscheidung</dt>
              <dd>
                Die Verarbeitung bleibt vollständig im Browser. Inhalte, Dateinamen und Ergebnisse
            ...
            <div>
              <dt>Umsetzung</dt>
              <dd>
                Nach der Funktionswahl werden die Dateien ausgewählt. Verbundene PDFs erhalten einen
                frei wählbaren Downloadnamen; umbenannte Dateien werden in einem ZIP-Archiv
                gebündelt.
              </dd>
            </div>
            <div>
              <dt>Grenzen</dt>
              <dd>
                Passwortgeschützte PDFs werden nicht verarbeitet. Bei sehr großen Dateien
                entscheidet der verfügbare Arbeitsspeicher des Geräts.
              </dd>
            </div>
          </dl>
        </section>
