...
export function CommunityAccount({ initialUser }: { initialUser: User | null }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === "register") {
      setVerificationEmail(String(data.email)); form.reset(); setMode("login");
  ...
  if (user) return <section className="community-account"><p className="eyebrow">Mitgliedschaft</p><h2>Willkommen, {user.displayName}.</h2><p>Dein Konto ist aktiv. Organisiere deine Bewerbungen in einer privaten Arbeitsfläche.</p><Link className="community-button" href="/community/bewerbungswerkstatt">Bewerbungswerkstatt öffnen</Link><button className="community-button secondary" onClick={logout}>Abmelden</button></section>;
  if (verificationEmail) return <section className="community-account" aria-labelledby="community-account-title"><p className="eyebrow">Mitgliedschaft</p><h2 id="community-account-title">E-Mail bestätigen.</h2><p>Wir haben dir einen Link gesendet. Prüfe auch deinen Spam-Ordner – der Link ist 24 Stunden gültig.</p>{message && <p role="status" aria-live="polite" className="community-error">{message}</p>}<button className="community-button secondary" onClick={resendVerification} disabled={busy}>{busy ? "Wird gesendet …" : "Bestätigungslink erneut senden"}</button><button className="community-button secondary" onClick={() => { setVerificationEmail(""); setMessage(""); }}>Zur Anmeldung</button></section>;
