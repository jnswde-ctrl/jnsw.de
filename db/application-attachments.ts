...
function bucket() {
  if (!env.APPLICATION_DOCUMENTS)
    throw new Error("R2 binding `APPLICATION_DOCUMENTS` is unavailable.");
  return env.APPLICATION_DOCUMENTS;
}
export async function listAttachments(userId: string, applicationId: string) {
  return getDb()
    .select()
    .from(applicationAttachments)
    .where(
      and(
        eq(applicationAttachments.userId, userId),
        eq(applicationAttachments.applicationId, applicationId),
      ),
    )
    .orderBy(desc(applicationAttachments.createdAt));
}
...
export async function getAttachment(userId: string, applicationId: string, attachmentId: string) {
  const item = await getDb().query.applicationAttachments.findFirst({
    where: and(
