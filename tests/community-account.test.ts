...
async function renderAccount(result: FetchResult) {
  ...
  await act(async () => { root.render(createElement(CommunityAccount, { initialUser: null })); });
...
test("resets the registration form after a successful asynchronous request", async () => {
  ...
  try {
    const { container, root } = await renderAccount({ status: 201, body: { message: "Bestätigungs-E-Mail gesendet." } });
...
for (const [name, result] of [
  ...
  ["failed verification email", { status: 503, body: { error: "Bestätigungs-E-Mail konnte nicht gesendet werden." } }],
] as const) {
